...
function check_collisions_classic() {
	...
	//vertical collision
	if (tile_meeting(x, y+spd.y, "Matrix"))
	{
		while (!tile_meeting(x, y+sign(spd.y), "Matrix"))
		{
			y = y + sign(spd.y);
		}
		spd.y = 0;
...
function check_collisions_pixel_perfect(_object = objBlock) { /// @description the pixel perfect collisions
	...
	if place_meeting(x + sign(spd.x), y, _object) {
		var wall = instance_place(x + sign(spd.x), y, _object);
		if (spd.x > 0)
		{ //right
	...
	y += spd.y;
	//Vertical collisions
	if place_meeting(x, y + sign(spd.y), _object) {
		var wall = instance_place(x, y + sign(spd.y), _object);
		if (spd.y > 0)
		{ //down
...
function check_collisions_tile_perfect() { /// @description the pixel perfect collisions
	...
	y += spd.y;
	//Vertical collisions
	if tile_meeting(x, y + sign(spd.y), "Matrix") {
		var wall = tilemap_get_at_pixel(tile, x, y + sign(spd.y));
		if (spd.y > 0)
		{ //down
			y = (wall.bbox_top - 1) - sprite_bbox_bottom;
		}
...
function tile_collisions(_tile)
{
	...
	if tile_meeting(x, y + spd.y, _tile)
	{
