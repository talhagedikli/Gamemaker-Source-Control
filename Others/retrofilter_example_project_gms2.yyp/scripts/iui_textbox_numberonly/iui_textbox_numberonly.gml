/// @description iui_textbox_numberonly(x, y, w, h, text, ID)
/// @param x
/// @param  y
/// @param  w
/// @param  h
/// @param  text
/// @param  ID
function iui_textbox_numberonly(argument0, argument1, argument2, argument3, argument4, argument5) {
	/**
	...
	**/
	...
	var em_          = string_width(string_hash_to_newline("M"));     // size of a M
	...
	var mustTrim    = (string_width(string_hash_to_newline(trimText)) > (insideWid - 8));
	...
	if (point_in_rectangle(iui_inputX, iui_inputY, bx, by, bx + bw, by + bh))
	{
	    ...
	    if (iui_activeItem == -1 && iui_inputDown)
	    {
	        ...
	        {
	            charInd++;
	            totalWidth += string_width(string_hash_to_newline(string_char_at(trimText, charInd)));
	        }
	        if (mustTrim)
	...
	var textEndX = string_width(string_hash_to_newline(string_copy(currentText, 1, displayChars - 1)));
	...
	if (mustTrim)
	{
	    ...
	    textOffX += string_width(string_hash_to_newline(tmpChar));
	...
	// cursor
	var cursorX = string_width(string_hash_to_newline(string_copy(currentText, 1, (cursorPos - showPos))));
	if (isFocus)
