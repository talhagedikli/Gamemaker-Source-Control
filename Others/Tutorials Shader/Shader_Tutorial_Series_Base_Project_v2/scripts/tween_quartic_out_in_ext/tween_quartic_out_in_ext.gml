/// @description tween_quartic_out_in_ext(input_value, input_range_min, input_range_max, output_range_min, output_range_max);
/// @param input_value
/// @param input_range_min
/// @param input_range_max
/// @param output_range_min
/// @param output_range_max
function tween_quartic_out_in_ext(argument0, argument1, argument2, argument3, argument4) {
	/*
	_______________________________________________________________________________
	INFO:
	-------------------------------------------------------------------------------
	This script remaps/tweens an input value from an input range to an output range
	in a "quartic out/in" fashion: slow -> fast -> slow
	...
	in_value /= in_range / 2;
	if (in_value < 1)
	return  out_range / 2 * in_value * in_value * in_value * in_value       + out_min;
	in_value -= 2;
	return -out_range / 2 * (in_value * in_value * in_value * in_value - 2) + out_min; */

	// Short version:
	var in_value = 2 * (argument0 - argument1) / (argument2 - argument1);
	if (in_value < 1)
	return  0.5 * (argument4 - argument3) * in_value * in_value * in_value * in_value       + argument3;
	in_value -= 2;
	return -0.5 * (argument4 - argument3) * (in_value * in_value * in_value * in_value - 2) + argument3;
