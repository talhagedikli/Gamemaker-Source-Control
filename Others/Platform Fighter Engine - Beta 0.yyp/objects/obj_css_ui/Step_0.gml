...
if (state == CSS_STATE.normal)
	...
	if (_sticklv != 0 && Virtual_Button_Hold(vc, _position, VB.sticklv))
		{
		replay_current = modulo(replay_current + _ud, array_length_1d(replay_list));
	...
	if (_confirm)
		...
		global.replay_record = false;
		Replay_Load(replay_list[replay_current]);
		Match_Begin(global.match_stage, global.match_stock, global.match_time, global.match_stamina);
		exit;
		}
	//Cancel
	else if (_cancel)
	...
	if (_sticklh != 0 && Virtual_Button_Hold(vc, _position, VB.sticklh))
		...
		if (match_settings_current == 0)
			{
			global.match_stock = modulo(global.match_stock + _rl, 100);
			}
		else if (match_settings_current == 1)
			{
			global.match_time = modulo(global.match_time + _rl, 61);
			}
		else if (match_settings_current == 2)
			{
			stamina_index = modulo(stamina_index + _rl, array_length_1d(possible_stamina_values));
			global.match_stamina = possible_stamina_values[@ stamina_index];
			}
		}
