function css_ui_player_window_draw() {
	switch(state)
		...
		case CSS_PLAYER_WINDOW_STATE.select_character:
			...
			palette_shader_simple_set(get_char_data(_char, CHAR_DATA.palette), _color);
			draw_sprite(get_char_data(_char, CHAR_DATA.CSS), 0, x, y);
			shader_reset();
		...
		case CSS_PLAYER_WINDOW_STATE.select_profile:
			...
			for(var i = 0; i < min(ds_list_size(global.profiles), 4); i++)
				{
		...
		case CSS_PLAYER_WINDOW_STATE.create_profile:
			...
			draw_set_alpha(0.25);
			draw_text(x - 32, y, profile_possible_chars[modulo(profile_new_letter - 2, array_length_1d(profile_possible_chars))]);
			draw_text(x + 32, y, profile_possible_chars[modulo(profile_new_letter + 2, array_length_1d(profile_possible_chars))]);
			draw_set_alpha(0.75);
			draw_text(x - 16, y, profile_possible_chars[modulo(profile_new_letter - 1, array_length_1d(profile_possible_chars))]);
			draw_text(x + 16, y, profile_possible_chars[modulo(profile_new_letter + 1, array_length_1d(profile_possible_chars))]);
			draw_set_alpha(1);
					...
					for(var i = 0; i < 4; i++)
						...
						//SCS
							...
							if (_n == 0)
								...
								draw_text(x + half_width - 4, y - half_height + _half + _y, Get_Input_Name(_scs_array[_n]));
								draw_set_halign(fa_center);
								}
							draw_set_alpha(1);
							}
		...
		case CSS_PLAYER_WINDOW_STATE.control_set:
			...
			draw_text(x, y - 16, Get_CC_Input_Name(_type, custom_controls_current) + ":");
			var _length = array_length_1d(custom_controls_array);
