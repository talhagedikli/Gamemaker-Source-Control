...
#region Special Airdodge exception for Melee Airdodge
	if (run && state_phase == 0 && airdodge_type == AIRDODGE.melee)
		{
		...
		if (button(INPUT.jump, buff))
			...
			if (stick_tilted(Lstick))
				{
				spd = airdodge_speed;
				dir = point_direction(0, 0, stick_get_value(Lstick, DIR.horizontal) * 10,
					stick_get_value(Lstick, DIR.vertical) * 10);
				}
			else
				{
				spd = 0;
		...
		switch(state_phase)
			...
			case 0:
				...
				if (state_frame == 0)
					...
					//Invulnerability
					hurtbox_set_inv(hurtbox, INV.invincible, roll_active);
					//Speed
					set_speed(roll_speed * state_direction, 0, false, false);
					}
				break;
				}
			case 1:
