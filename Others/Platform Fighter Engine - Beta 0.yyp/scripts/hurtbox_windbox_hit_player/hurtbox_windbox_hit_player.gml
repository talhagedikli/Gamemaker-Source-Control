			...
			if (_hitbox.base_knockback != 0)
				...
				//Windbox speed boost
					...
					if (on_ground() && !_hitbox.windbox_lift)
						{
						set_speed(_hsp, 0, _hitbox.windbox_accelerate, true);
						//Max speed
						...
						{
						//Set the speed
						set_speed(_hsp, _vsp, _hitbox.windbox_accelerate, _hitbox.windbox_accelerate);
						//Maximum speed
						if (_hitbox.windbox_max_speed > 0 && point_distance(0, 0, hsp,vsp) > _hitbox.windbox_max_speed)
							{
							var _dir = point_direction(0, 0, hsp, vsp);
							set_speed(lengthdir_x(_hitbox.windbox_max_speed, _dir), lengthdir_y(_hitbox.windbox_max_speed, _dir), false, false);
							}
						}
					}
				}
