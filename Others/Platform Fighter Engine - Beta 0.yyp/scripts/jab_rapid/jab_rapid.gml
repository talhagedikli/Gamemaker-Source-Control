		...
		switch(_phase)
			...
			case 0:
				...
				if (attack_frame == 0)
					...
					var _hitbox = create_melee(20, 0, 0.5, 0.3, 1, 6, 0.1, 5, 0, 4, HITBOX_SHAPE.circle, 0);
			...
			case 3:
				...
				if (attack_frame == 0)
					...
					attack_phase++;
					attack_frame = 4;
					set_speed(facing * 2, 0, false, false);
					var _hitbox = create_melee(20, 5, 0.7, 0.3, 1, 4, 0, 3, 0, 4, HITBOX_SHAPE.circle, 1);
			...
			case 5:
				...
				if (button(INPUT.attack, 12)) or (attack_has_hit() && button_hold(INPUT.attack))
					...
					attack_phase++;
					attack_frame = 4;
					set_speed(facing * 5, 0, true, false);
					}
				//Auto end
				else if (attack_frame == 0)
					{
			...
			case 6:
				...
				if (attack_frame == 0)
					...
					attack_phase++;
					attack_frame = 240;
					var _hitbox = create_melee(30, 0, 0.9, 0.7, 1, 1, 0.1, 1, 45, 2, HITBOX_SHAPE.circle, 2, FLIPPER.sakurai);
			...
			case 7:
				...
				if (button_hold(INPUT.attack) || attack_frame > 220)
					...
					if (attack_frame % 3 == 0)
						...
						var _hitbox = create_melee(38, 0, 0.9, 0.7, 1, 1, 0.1, 1, 45, 2, HITBOX_SHAPE.circle, 2, FLIPPER.sakurai);
						_hitbox.hit_fx_style = HIT_FX.normal_weak;
						_hitbox.techable = false;
						_hitbox.di_angle = 0;
						_hitbox.asdi_multiplier = 1.5;
				...
				if ((!button_hold(INPUT.attack) || attack_frame <= 0) && attack_frame <= 220)
					...
					attack_phase++;
					attack_frame = 10;
					set_speed(facing * 5, 0, false, false);
					}
				break;
				}
			//Third Jab Final Hit
			case 8:
				...
				if (attack_frame == 9)
					...
					attack_frame = 24;
					attack_phase++;
					var _hitbox = create_melee(40, -10, 0.7, 1.3, 6, 10, 0.45, 13, 45, 2, HITBOX_SHAPE.circle, 3);
					_hitbox.hit_fx_style = HIT_FX.normal_strong;
					_hitbox.hit_sfx = snd_hit_strong;
					var _hitbox = create_melee(10, 0, 0.5, 0.5, 6, 10, 0.4, 13, 55, 2, HITBOX_SHAPE.circle, 3);
					_hitbox.hit_fx_style = HIT_FX.normal_strong;
