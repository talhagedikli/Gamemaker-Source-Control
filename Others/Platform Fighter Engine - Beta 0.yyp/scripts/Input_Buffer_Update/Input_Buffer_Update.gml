...
#region Keyboard
	if (device_type == DEVICE.keyboard)
		...
		for(var m = 0; m < array_length_1d(custom_controls); m += 2)
			{
		...
		if (_run_button)
			{
			if (control_tilted_l == 0) then control_flicked_l = 0;
			}
		}
#endregion 
#region Controller
			...
			if (_input < INPUT.length)
				...
				if (_input == CC_INPUT_CONTROLLER.short_hop)
					...
					if (controller_check_input(device, _control, false))
						{
						if (scs_switch_sticks)
							{
							if (control_tilted_r == 0) then control_flicked_r = 0;
							}
						else
							{
							if (control_tilted_l == 0) then control_flicked_l = 0;
