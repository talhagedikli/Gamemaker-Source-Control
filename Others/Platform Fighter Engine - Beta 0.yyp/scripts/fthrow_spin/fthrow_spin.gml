	...
	if (run)
		...
		switch(_phase)
			...
			case PHASE.start:
				...
				hurtbox_set_inv(hurtbox, INV.superarmor, 34);
				with(grabbed_id)
					{
					hurtbox_set_inv(hurtbox, INV.superarmor, 33);
					}
				return;
				}
			//Startup -> Active
			case 0:
				...
				if (attack_frame == 0)
					...
					attack_phase++;
					attack_frame = 18;
					set_speed(2 * facing, 0, true, false);
					var _hitbox = create_targetbox(45, 0, 1, 1, 10, 11, 0.95, 7, 45, 1, HITBOX_SHAPE.square, 0, grabbed_id);
					_hitbox.hit_fx_style = [HIT_FX.normal_strong, HIT_FX.lines, HIT_FX.shield, HIT_FX.explosion];
