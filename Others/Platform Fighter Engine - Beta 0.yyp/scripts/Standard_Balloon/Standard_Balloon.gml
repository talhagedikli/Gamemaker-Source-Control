...
#region End Hitstun - Go to tumble
	if (run && state_frame == 0)
		...
		set_speed(hsp * balloon_end_speed_multiplier, vsp * balloon_end_speed_multiplier, false, false);
...
#region Balloon knockback
	...
	var _scale = clamp((state_frame / _hitstun_start_frame) * (kspd / 10), 1, 2);
	...
	repeat(_times)
		...
		//Cloud trails
		if (run && knockback_cloud_trails_enable && kspd > 9)
			{
			...
			with(_cloud)
				...
				var _color = palette_get_color(other.palette, 0);
				color =
					[
