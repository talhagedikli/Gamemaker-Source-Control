...
function Spawn_Players() {
	...
	for(var i = 0; i < min(global.number_of_players, _n); i++)
		{
		...
		with(instance_create_layer(_s.x, _s.y, _s.layer, obj_player))
			...
			var _data = global.players[@ i];
			var _character_number = _data[PLAYER_DATA.character];
			var _char = global.all_character_data[_character_number];
			character = _data[PLAYER_DATA.character];
			character_script = _char[CHAR_DATA.script];
