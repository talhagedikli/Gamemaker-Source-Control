			...
			case 0:
				...
				if (attack_frame == 0)
					...
					if (!on_ground())
						...
						{
						set_speed(facing * 6, 0, true,true);
						}
				
					var _hitbox = create_melee(15, -2, 1.3, 0.8, 20, _attack_power, 1.3, 30, 35, 5, HITBOX_SHAPE.circle, 0);
					hitbox_set_property(_hitbox, HITBOX_PROPERTY.hit_sfx, snd_hit_explosion);
					hitbox_set_property(_hitbox, HITBOX_PROPERTY.hit_fx_style, [HIT_FX.normal_strong, HIT_FX.emphasis]);
					hitbox_set_property(_hitbox, HITBOX_PROPERTY.techable, false);
					hitbox_set_property(_hitbox, HITBOX_PROPERTY.shieldstun_scaling, 1);
