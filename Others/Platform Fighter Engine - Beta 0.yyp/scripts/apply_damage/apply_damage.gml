...
function apply_damage() {
	...
	if (show_true_combos)
		...
		if (_id.state == PLAYER_STATE.in_balloon ||
			...
			{
			with(instance_create_layer(x + irandom_range(-14, 14), y -32 + irandom_range(-4, 4), layer, obj_notice))
				{
				image_blend = c_yellow;
				text = "COMBO";
				font = fnt_notice_text;
				lifetime = 70;
				}
			}
		else
			...
			with(instance_create_layer(x + irandom_range(-14, 14), y -32 + irandom_range(-4, 4), layer, obj_notice))
				{
				image_blend = c_red;
				text = Get_Player_State_Name(_id.state);
				font = fnt_notice_text;
				lifetime = 70;
				}
			}
