///Sets all of the variables for a character 2
function character1() {
	//Collision box
	...
	//Attacks
	my_attacks[? "Jab"	 ] = jab0;
	my_attacks[? "DashA" ] = dashattack_corrin;
	...
	my_attacks[? "Nair"	 ] = nair_palutena;
	my_attacks[? "Fair"	 ] = fair_samus;
	my_attacks[? "Bair"	 ] = bair_k_rool;
	my_attacks[? "Uair"	 ] = uair_joker;
	my_attacks[? "Dair"	 ] = dair_yoshi;
			 
	my_attacks[? "Nspec" ] = [nspec_charge_shot, nspec_charge_shot_draw];
	my_attacks[? "Fspec" ] = fspec_wind;
	my_attacks[? "Uspec" ] = uspec_bowser;
	my_attacks[? "Dspec" ] = dspec_counter;
			 
	my_attacks[? "Grab"	 ] = grab_tether;
	my_attacks[? "DashG" ] = dash_grab_tether;
	...
	my_sprites[? "Entrance"	] = define_anim(spr_entrance0, define_anim_ext(spr_idle1, 0, 0.3));
	my_sprites[? "Idle"		] = define_anim_ext(spr_idle1, 0, 0.3);
