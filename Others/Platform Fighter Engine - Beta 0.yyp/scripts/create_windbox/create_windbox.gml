///@func create_windbox(rel_x,rel_y,xscale,yscale,damage,knockback,angle,lifetime,shape,hitbox_group,[flipper],[multihit],[accelerate],[push],[max_speed],[lift]);
///@param rel_x
///@param rel_y
///@param xscale
///@param yscale
...
///@param [max_speed]
///@param [lift]
function create_windbox() {
	var _newhitbox = instance_create_layer(x + argument[0] * facing, y + argument[1], layer, obj_windbox);
	with(_newhitbox)
		{
		...
		lifetime = argument[7];
		hitbox_set_property(_newhitbox, HITBOX_PROPERTY.shape, argument[8]);
		owner = other.id;
