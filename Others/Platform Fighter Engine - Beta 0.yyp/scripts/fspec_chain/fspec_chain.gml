	...
	if (run)
		...
		switch(_phase)
			...
			case PHASE.start:
				...
				attack_frame = 17;
				fspec_chain_segments = [];
				fspec_chain_repeat = 0;
				return;
				}
			//Startup -> Active
			case 0:
			...
			case 1:
				...
				if (attack_frame == 6)
					...
					repeat(12)
						{
						var _len = array_length_1d(fspec_chain_segments);
						var _seg = [];
						...
						if (_len == 11)
							{
							var _hitbox = create_melee(_seg[0], _seg[1], 0.2, 0.2, 2, 1, 0.4, 4, 120, -1, HITBOX_SHAPE.square, 0);
							...
							{
							var _hitbox = create_melee(_seg[0], _seg[1], 0.1, 0.1, 1, 1, 0.4, 3, 45, -1, HITBOX_SHAPE.square, 0);
							_hitbox.hit_sfx = snd_hit_slap2;
							_hitbox.asdi_multiplier = 2;
							_hitbox.drift_di_multiplier = 2;
							}
						_seg[4] = _hitbox;
						fspec_chain_segments[@ _len] = _seg;
						}
					anim_frame = 3;
					}
				...
				//Chain logic
				fspec_chain_repeat = max(--fspec_chain_repeat, 0)
				var _num = array_length_1d(fspec_chain_segments);
				if (_num > 0)
					{
					if (stick_tilted(Lstick))
						{
						var _seg = fspec_chain_segments[@ 0];
						...
						if (stick_flicked(Lstick))
							{
							//Reset hitboxes
							if (fspec_chain_repeat == 0)
								{
								hitbox_group_reset_all();
								fspec_chain_repeat = 10;
								}
							for(var m = 1; m < _num; m++)
								{
								var _seg = fspec_chain_segments[@ m];
				...
				for(var i = 1; i < _num; i++)
					...
					var _pre = fspec_chain_segments[@ i - 1];
					//Movement & Collisions
					_seg[@ 2] = approach(_seg[2], 0, 0.1);
					_seg[@ 3] = min(_seg[3] + 0.35, 6);
