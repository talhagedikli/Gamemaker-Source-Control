...
for(var i = 0; i < global.number_of_players; i++)
	{
	var _is_cpu = get_player_data(i, PLAYER_DATA.is_cpu);
	if (_is_cpu) then continue;
	
	var _device = get_player_data(i, PLAYER_DATA.device);
	var _device_type = get_player_data(i, PLAYER_DATA.device_type);
...
if (_inst != noone)
	...
	switch(_inst.stage)
		...
		case rm_Eight: stage_name_label.text = "Battlefield 2"; break;
