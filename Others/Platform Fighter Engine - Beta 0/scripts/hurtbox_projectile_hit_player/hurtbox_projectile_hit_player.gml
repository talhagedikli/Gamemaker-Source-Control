...
switch(_hurtbox.inv_type)
	...
	case INV.normal:
		...
		if (_hitbox.base_knockback != 0)
			{
			if (is_finishing_blow(_total_kb, x, y, _calc_angle, stored_hitstun, id))
				{
				_total_hl += finishing_blow_hitlag_increase;
				var _fx = create_fx(spr_hit_fx_final_hit, 1, 0, 34, x, y, 2, irandom(360));
				_fx.image_blend = c_black;
				//Final Hit fade
				background_clear_activate(_total_hl, palette_get_color(_hitbox.owner.palette, 0));
				}
			}
	...
	case INV.shielding:
		...
		if (shield_pushback_enable)
			{
			//Angle
			_calc_angle = apply_angle_flipper(_hitbox.angle, _hitbox.angle_flipper, _hitbox.owner, s_angle_knockback_threshold, _hitbox.facing);
			//Setting speed
	...
	case INV.powershielding:
		...
		state_frame = 0;
		//Effects
		create_fx(spr_hit_powershield, 1, 0, 12, x, y, 3, choose(-1, 1), "FX_Layer_Below");
		create_fx_action_lines(10, x, y, irandom(10));
