...
if (run)
	...
	switch(_phase)
		...
		case 0:
			...
			if (attack_frame <= 34 && !button_hold(INPUT.special))
				{
				anim_frame = 9;
				...
				var _proj = custom_projectile(obj_nspec_bow_arrow, 24, 0, 0.2, 0.2, _damage, _kb, _scaling, 45, 180, HITBOX_SHAPE.circle, _hsp, 0, FLIPPER.sakurai);
				_proj.destroy_on_blocks = true;
				_proj.destroy_on_hit = !_fully_charged;
				_proj.grav = _grav;
				_proj.base_hitlag = _hitlag;
				_proj.hit_sfx = _fully_charged ? snd_hit_strong2 : snd_hit_weak2;
				_proj.hit_fx_style = _fully_charged ? HIT_FX.explosion : HIT_FX.normal_medium;
				_proj.knockback_state = _fully_charged ? PLAYER_STATE.in_balloon : PLAYER_STATE.in_hitstun;
