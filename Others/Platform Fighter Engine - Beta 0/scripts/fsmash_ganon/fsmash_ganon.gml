	...
	switch(_phase)
		...
		case 1:
			...
			if (attack_frame == 0)
				...
				var _hitbox = create_melee(-8, -32, 1, 0.8, _damage, 4, 2, 15, 60, 3, HITBOX_SHAPE.circle, 0, FLIPPER.from_player_center_horizontal);
		...
		case 2:
			...
			if (attack_frame == 6)
				...
				var _hitbox = create_melee(32, -20, 2, 0.8, _damage, 8, 1.2, 15, 45, 3, HITBOX_SHAPE.circle, 0, FLIPPER.sakurai);
				_hitbox.hit_fx_style = HIT_FX.normal_strong;
			...
			if (attack_frame == 4)
				...
				if (charge >= smash_attack_max_charge)
					{
					var _hitbox = create_melee(42, 0, 2, 0.8, 50, 8, 1.2, 20, 35, 4, HITBOX_SHAPE.circle, 2);
					_hitbox.hit_fx_style = HIT_FX.explosion;
					_hitbox.hit_sfx = snd_hit_explosion2;
					_hitbox.knockback_state = PLAYER_STATE.in_balloon;
					var _hitbox = create_melee(48, 24, 4, 0.2, 5, 8, 0.7, 6, 90, 4, HITBOX_SHAPE.square, 2);
					_hitbox.hit_fx_style = HIT_FX.explosion;
					_hitbox.hit_sfx = snd_hit_explosion2;
					}
				else
					{
					var _damage = calculate_smash_damage(20);
					var _hitbox = create_melee(42, 0, 2, 0.8, _damage, 8, 1, 20, 45, 4, HITBOX_SHAPE.circle, 0, FLIPPER.sakurai);
					_hitbox.hit_fx_style = HIT_FX.normal_strong;
					_hitbox.hit_sfx = snd_hit_strong2;
					_hitbox.shieldstun_scaling = 0.5;
					_hitbox.knockback_state = PLAYER_STATE.in_balloon;
