	...
	switch(_phase)
		...
		case 0:
			...
			if ((charge >= smash_attack_max_charge || (!button_hold(INPUT.smash) && !button_hold(INPUT.attack) && !button_hold(INPUT.special))) && attack_frame == 0)
				...
				var _hitbox = create_melee(0, 4, 0.4, 0.6, _damage, 9, 0.9, 15, 90, 2, HITBOX_SHAPE.circle, 0);
				...
				var _hitbox = create_melee(0, -38, 0.3, 0.4, _damage, 10, 1, 15, 90, 2, HITBOX_SHAPE.circle, 0);
				_hitbox.hit_fx_style = [HIT_FX.explosion, HIT_FX.slash2];
		...
		case 1:
			...
			if (attack_frame == 6)
				...
				anim_frame = 4;
				var _damage = calculate_smash_damage(10);
				var _hitbox = create_melee(20, 6, 0.7, 0.6, _damage, 9, 0.9, 5, 75, 5, HITBOX_SHAPE.circle, 0, FLIPPER.from_player_center_horizontal);
				_hitbox.hit_fx_style = HIT_FX.normal_strong;
