	...
	switch(_phase)
		...
		case 0:
			...
			if (attack_frame == 0)
				...
				//Detection hitbox
				create_detectbox(16, 0, 0.6, 0.6, 9, HITBOX_SHAPE.circle, 0);
				//VFX
		...
		case 1:
			...
			if (attack_frame % 3 == 0)
				{
				var _fx = create_fx(spr_buster_wolf_trail, 1, 0, 13, x, bbox_bottom, 2, 0, "FX_Layer_Below");
				_fx.fx_xs = 2 * facing;
				}
			...
			if (attack_frame == 0)
				{
				anim_frame = 8;
				set_speed(8 * facing, 0, false, false);
				attack_phase = 3;
		...
		case 2:
			...
			if (attack_frame == 20)
				...
				if (on_ground())
					{
					var _hitbox = create_melee(56, 0, 1.1, 0.7, 20, 5, 1.6, 15, 40, 4, HITBOX_SHAPE.square, 1);
					...
					{
					var _hitbox = create_melee(56, 0, 1.1, 0.7, 19, 5, 1.2, 8, 45, 4, HITBOX_SHAPE.square, 1);
					_hitbox.hit_sfx = snd_hit_blast;
					_hitbox.hit_fx_style = HIT_FX.explosion;
					}
				var _hitbox = create_melee(108, 0, 3, 1.8, 5, 7, 0.6, 3, 50, 5, HITBOX_SHAPE.square, 1);
				_hitbox.hit_sfx = snd_hit_blast2;
				_hitbox.hit_fx_style = HIT_FX.normal_medium;
				
				//VFX
				var _fx = create_fx(spr_buster_wolf_explosion, 1, 0, 39, x + (48 * facing), y, 2, 0, "FX_Layer_Below");
				_fx.fx_xs = 2 * facing;
				}
