	...
	switch(state)
		...
		case CSS_PLAYER_WINDOW_STATE.select_profile:
			...
			if (_sticklv != 0 && Virtual_Button_Hold(obj_css_ui.vc, _position, VB.sticklv))
				{
				profile_current = modulo(profile_current + _ud, ds_list_size(global.profiles));
				...
				if (!Profile_Get(profile_current, PROFILE.autogenerated))
					...
					if (!_used)
						...
						with(obj_css_player_window)
							...
							profile_scroll = 0;
							profile_current = modulo(profile_current, ds_list_size(global.profiles));
							}
						//Save profiles
						Profile_Save_All();
						}
		...
		case CSS_PLAYER_WINDOW_STATE.create_profile:
			...
			if (_sticklh != 0 && Virtual_Button_Hold(obj_css_ui.vc, _position, VB.sticklh))
				{
				profile_new_letter = modulo(profile_new_letter + _rl, array_length_1d(profile_possible_chars));
				}
		...
		case CSS_PLAYER_WINDOW_STATE.controls:
			...
			if (_confirm)
				{
				//Inputs
				if (custom_controls_current < CC_INPUT_CONTROLLER.length)
					{
		...
		case CSS_PLAYER_WINDOW_STATE.control_set:
			...
			if (_any_input)
				...
				if (_device_type == DEVICE.controller)
					...
					if (array_length_1d(custom_controls_array) < 4)
						{
						_currently_held = controller_get_input(_device);
						for(var i = 0; i < array_length_1d(_currently_held); i++)
							{
							var _exists = false;
							for(var m = 0; m < array_length_1d(custom_controls_array); m++)
								{
							...
							if (!_exists)
								{
								custom_controls_array[array_length_1d(custom_controls_array)] = _currently_held[i];
								}
							}
						}
					}
