...
setDefaultVariables = function()
{
	...
	accel				= 0.3;
	decel				= 0.2;
	wepon.delay 		= weponDelay;
}

playShootSound = function()
{
	var a = audio_play_sound(aShoot, 2, false);
...
state.add("move", {
	...
	step: function()
	{
		...
		if (instance_exists(b) && !b.state.state_is("death"))
		{
			if (b.owner.object_index == objEnemies)
			{
				b.destroy();
				state.change("death");
			}
	...
	step: function()
	{
		image_alpha = flerp(image_alpha, 0.5, 0.05);
	 	...
	 	ghostTimer.on_timeout(function()
	 	{
			...
			part_type_gravity(global.ptBolis, 0.2, 180 - shipDir.angle());
			// part_type_speed(global.ptBolis, maxSpd, maxSpd, 0.5, 0);
...
state.add("death", {
	enter: function()
	{
		...
		audio_play_sound(aLose, 3, false);
